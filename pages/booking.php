...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Fungsi untuk set menu aktif berdasarkan halaman saat ini
    function setActiveMenu() {
        ...
        const currentPage = currentPath.split('/').pop() || 'beranda.html';
        
        console.log('Current page:', currentPage); // Untuk debugging
        
        // Cari link yang sesuai dengan halaman saat ini
        let foundActive = false;
        
        // Cek terlebih dahulu dropdown items
        dropdownItems.forEach(item => {
            const href = item.getAttribute('href');
            if (href === currentPage) {
                item.classList.add('active');
                // Aktifkan parent dropdown juga
                const parentDropdown = item.closest('.dropdown');
                if (parentDropdown) {
                    const dropdownToggle = parentDropdown.querySelector('.dropdown-toggle');
                    dropdownToggle.classList.add('active');
                    foundActive = true;
                    console.log('Found in dropdown:', href); // Untuk debugging
                }
            }
        });
        
        // Jika tidak ditemukan di dropdown items, cek di nav links biasa
        if (!foundActive) {
            allNavLinks.forEach(link => {
                const href = link.getAttribute('href');
                if (href === currentPage && !link.classList.contains('dropdown-toggle')) {
                    link.classList.add('active');
                    console.log('Found in nav links:', href); // Untuk debugging
                }
            });
        }
        
        // Logika khusus untuk halaman booking.html
        // Karena booking.html adalah bagian dari Peminjaman Lab
        if (currentPage === 'booking.html') {
            // Aktifkan dropdown Peminjaman Lab
            const peminjamanDropdown = document.querySelector('.nav-item.dropdown:nth-child(4)');
            if (peminjamanDropdown) {
                const dropdownToggle = peminjamanDropdown.querySelector('.dropdown-toggle');
                dropdownToggle.classList.add('active');
                console.log('Activated Peminjaman Lab dropdown for booking.html'); // Untuk debugging
            }
        }
    }
    ...
    allNavLinks.forEach(link => {
        link.addEventListener('click', function (e) {
            // Hanya gunakan preventDefault untuk dropdown toggle di mobile
            if (this.classList.contains('dropdown-toggle') && window.innerWidth <= 992) {
                e.preventDefault();
                const dropdownMenu = this.nextElementSibling;
                dropdownMenu.classList.toggle('show');
                
                // Tutup dropdown lainnya jika terbuka
            ...
            // Untuk link biasa, biarkan browser menavigasi secara normal
            // Status active akan diatur ulang saat halaman baru dimuat
        });
    });
    ...
    dropdownItems.forEach(item => {
        item.addEventListener('click', function (e) {
            // Biarkan browser menavigasi secara normal
            // Link akan berfungsi dan halaman akan reload
            // setActiveMenu() akan dipanggil lagi saat halaman baru dimuat
        });
    });
    ...
    // Untuk desktop: tutup dropdown saat mouse leave
    if (window.innerWidth > 992) {
    ...
    // Kode untuk calendar (tetap sama seperti sebelumnya)
